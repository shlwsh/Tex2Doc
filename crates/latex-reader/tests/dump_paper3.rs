...
fn dump_paper3_blocks() {
    ...
    let mut entries: std::collections::BTreeMap<PathBuf, Vec<u8>> =
    ...
    for i in 0..archive.len() {
        ...
        entries.insert(PathBuf::from(name), buf);
    }
    ...
    for (i, b) in doc.blocks.iter().enumerate() {
        let label = match b {
            ...
            doc_semantic_ast::Block::List { is_ordered, items, .. } => {
                format!("L(o={is_ordered},n={})", items.len())
            }
            doc_semantic_ast::Block::Equation { latex, is_block, .. } => format!(
                "E(b={is_block},{})",
