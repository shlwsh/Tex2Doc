...
pub fn write_markdown(report: &QualityReport, out: &Path) -> std::io::Result<()> {
    ...
    if let Some(layer) = report
        .layer_results
        .iter()
        .find(|l| l.layer == crate::layer::Layer::Structural)
    {
    ...
    s.push_str(&format!(
        ...
        if passed_ratio(0.75, report.char_ratio_docx_to_oracle) {
            "✓"
        } else {
            "✗"
        }
    ...
    s.push_str(&format!(
        ...
        if passed_ratio(0.75, report.char_ratio_rust_to_oracle) {
            "✓"
        } else {
            "✗"
        }
    ...
    if let Some(layer) = report
        .layer_results
        .iter()
        .find(|l| l.layer == crate::layer::Layer::Visual)
    {
        s.push_str("| # | 名称 | 期望 | 实际 | 状态 |\n");
        s.push_str("|---|------|------|------|------|\n");
        for (i, c) in layer.checks.iter().enumerate() {
