...
\appendix
\section{DSB-Lite 部署细节}
\label{app:dsb-deploy}
...
\subsection{部署脚本}
本机原生部署，详见 \texttt{deploy/deathstarbench/host\_native/}：
\begin{itemize}
\item \texttt{start\_services.sh}：启动 Redis + 8 个 Flask 微服务
\item \texttt{stop\_services.sh}：停止所有服务
\item \texttt{start\_gateway.sh}：启动 OpenResty/nginx 网关（端口 8080）
\item 总占用内存约 800 MB（含 Redis），远低于 3.5 GB 预算
\end{itemize}
