...
- Left: `examples/paper3/output/to-docx/v132-20260618-134854-论文稿件-jos-rust.docx`
...
| Delete | 6 | - | - | JOSKeywords | - | 关键词:微服务;分布式日志采集;动态关注度评分;定向过滤;参数敏感性; Grafana Loki |
...
| Insert | - | 6 | - | - | JOSKeywords | 关键词: 微服务; 分布式日志采集; 动态关注度评分; 定向过滤; 参数敏感性; Grafana Loki |
| Delete | 8 | - | - | JOSCitation | - | 中文引用格式: 石洪雷,赵涓涓.基于动态关注清单的微服务日志定向采集方法.软件学报. http://www.jos.org.cn/1000-9825/0000.htm |
| Delete | 9 | - | - | JOSCitation | - | 英文引用格式: Shi HL, Zhao JJ. Dynamic attention list-based directed log collection method for microservices. Ruan Jian Xue Bao/Journal of Software (in Chinese). http://www.jos.org.cn/1000-9825/0000.htm |
...
| Delete | 161 | - | - | JOSCode | - | Score(u,t)=α Freq(u,t)+β Err(u,t)+γ Delay(u,t)+δ Trend(u,t) (1) |
| Delete | 162 | - | - | JOSBody | - | 其中 α+β+γ+δ=1（均非负），四个因子分别定义为： |
| Delete | 163 | - | - | JOSBody | - | itemize Freq(u,t)=count(u)/max v count(v)，归一化频次； Err(u,t)=err_count(u)/count(u)，错误率； Delay(u,t)=min(1, rt(u)/T)，延迟严重度； Trend(u,t)=σ\!((Freq t -Freq t-1 )/max(Freq t-1 ,ε))，热度趋势（σ 为 sigmoid 函数，将变化率映射到(0,1)）。 |
...
| Delete | 165 | - | - | JOSCode | - | wdecay(u,t)=Σi wi·exp(-λ (t-ti)) (2) |
| Delete | 166 | - | - | JOSBody | - | 其中 λ 为衰减系数（默认 0.1/s），ti 为第 i 个历史窗口的时间戳。 |
| Delete | 167 | - | - | JOSBody | - | 参数自适应。当全局错误率超过 10%时，β 自动提升 50%；当全局平均延迟超过 2T 时，γ 自动提升 50%；提升后重新归一化，确保权重系数之和为 1。默认均衡模式取 α=0.3，β=0.3，γ=0.2，δ=0.2。 |
...
| Delete | 169 | - | - | JOSCode | - | Input: 流量日志集合 L，策略 S=(T,E,K,TTL)，历史窗口 ℋ |
| Insert | - | 171 | - | - | - | Input: 流量日志集合 L，策略 S=(T,E,K,TTL)，历史窗口 mathcalH |
| Delete | 171 | - | - | JOSCode | - | \| )，历史窗口{H \| \| $ \| \| H ← ∅ \| ForEach \| ForEach () \| End \| end \| \| { \| If \| If () \| End \| end \| \| (l) > T$ {or \| \| ${status \| \| (l) E$ \| \| { \| \| $p {Generalize \| \| (l.{url \| \| )$ {数字→{id}，UUID→{uuid\ \| \| $H[p].{count \| \| ... |
| Insert | - | 173 | - | - | - | 1 |
| Insert | - | 174 | - | - | - | H ← ∅; |
| Insert | - | 175 | - | - | - | 2 |
...
| Insert | - | 204 | - | - | - | 14 |
| Insert | - | 205 | - | - | - | return A; |
| Delete | 173 | - | - | JOSBody | - | 对任意 URL 模式 u 和时间 t，Score(u,t)∈[0,1]。 |
| Delete | 174 | - | - | JOSBody | - | 四个因子 Freq、Err、Delay、Trend 的值域均为[0,1]（Trend 经 sigmoid 映射后属于(0,1)⊂[0,1]）。权重系数满足 α,β,γ,δ≥ 0 且 α+β+γ+δ=1。由凸组合性质，Score=α f1+β f2+γ f3+δ f4∈[0,1]。 |
| Delete | 175 | - | - | JOSBody | - | 当 δ=0（无趋势因子）且 λ=0（无时间衰减）时，DASM 退化为静态加权 Top-K。 |
| Delete | 176 | - | - | JOSBody | - | URL 泛化规则包括：纯数字路径段映射为{id}，UUID 或长哈希段映射为{uuid}，查询参数按键名归一化。如果遇到异常极稀疏的窗口，算法会退化为仅保留 ERROR 级别的兜底规则。后文微基准测试显示，处理 5000 条合成日志大约耗时 15 ms。和 Promtail 依赖静态标签配置不同，DASM 以网关 access log 为策略输入，通过趋势感知和时间衰减动态识别高价值 URL 模式，避免历史噪声污染清单。 |

## 格式差异

| Left# | Right# | 文本 | 字段变化 |
| ---: | ---: | --- | --- |
| 112 | 114 | 72 vs 4388 条* | runs: `style=-;b=false;i=false;u=-;va=-;sz=15;font=Times New Roman/宋体 x1` -> `style=-;b=false;i=false;u=-;va=-;sz=15;font=Times New Roman/宋体 x1; style=-;b=false;i=false;u=-;va=superscript;sz=15;font=Times New Roman/宋体 x1` |
| 114 | 116 | 0.05% CPU* | runs: `style=-;b=false;i=false;u=-;va=-;sz=15;font=Times New Roman/宋体 x1` -> `style=-;b=false;i=false;u=-;va=-;sz=15;font=Times New Roman/宋体 x1; style=-;b=false;i=false;u=-;va=superscript;sz=15;font=Times New Roman/宋体 x1` |
| 168 | 170 | Algorithm 1: 基于 DASM 的关注清单动态生成 | runs: `style=-;b=true;i=false;u=-;va=-;sz=-;font=-/- x1` -> `style=-;b=false;i=false;u=-;va=-;sz=18;font=Times New Roman/宋体 x1; style=-;b=true;i=false;u=-;va=-;sz=18;font=Times New Roman/宋体 x1` |
| 170 | 172 | Output: 关注清单 A | paragraph.style: `JOSCode` -> `-`<br>runs: `style=-;b=false;i=false;u=-;va=-;sz=-;font=-/- x1; style=-;b=true;i=false;u=-;va=-;sz=-;font=-/- x1` -> `style=-;b=false;i=false;u=-;va=-;sz=18;font=Times New Roman/宋体 x1; style=-;b=true;i=false;u=-;va=-;sz=18;font=Times New Roman/宋体 x1` |

## OOXML Hash

| Part | Equal | Left hash | Right hash |
| --- | --- | --- | --- |
| word/document.xml | false | eccbc8987230f0ab | abef806ccb0811fd |
