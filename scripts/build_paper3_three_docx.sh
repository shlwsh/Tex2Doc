...

{
  ...
  echo
  grep -E '^(backend-(requested|selected|fallback-from|reason)|profile-(id|page-setup)|reference-(labels|edges)|citations|unresolved-references|bookmarks|hyperlinks):' "$SEMANTIC_LOG" || true
} >>"$REPORT"
